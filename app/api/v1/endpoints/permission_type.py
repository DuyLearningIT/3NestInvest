...
async def create_permission_type(db: db_depend, request : PermissionTypeCreate, admin = Depends(admin_required)):
	response = await permission_type_crud.create_permission_type(db, request)
	return response

@router.post('/update-permission-type')
async def update_permission_type(db: db_depend, request: PermissionTypeUpdate, admin = Depends(admin_required)):
...
async def delete_permission_type(db: db_depend, request_id : int, admin = Depends(admin_required)):
