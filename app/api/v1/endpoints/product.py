...
async def create_product(db: db_depend, request: CreateProduct, logRequest: Request, admin=Depends(admin_required)):
	response = await product_crud.create_product(db, request, logRequest, admin)
	return response

# Admin required
@router.post('/update-product')
async def update_product(db: db_depend, request: UpdateProduct, logRequest: Request, admin=Depends(admin_required)):
	response = await product_crud.update_product(db, request, logRequest, admin)
	return response

# Admin required
@router.delete('/delete-product')
async def delete_product(db: db_depend, product_id : int, logRequest: Request, admin = Depends(admin_required)):
	response = await product_crud.delete_product(db, product_id, logRequest, admin)
	return response
...
async def get_products_by_role_and_type(db: db_depend, role_id: int, type_id : int, logRequest: Request, admin = Depends(admin_required)):
	response = await product_crud.get_products_by_role_and_type(db, role_id, type_id, logRequest, admin)
	return response
